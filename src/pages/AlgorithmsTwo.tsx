    ...
    return <div className = 'w-full h-full flex flex-col items-center gap-[5px]'>
        ...
        <InfoDropdown outerString = 'Mahony Filter'>
            <div className = 'flex flex-col justify-items items-center'>
                ...
                <p className = 'w-full'>Now, we can get our estimated true angular velocity via <InlineMath>{'\\Omega_{true} \\approx \\Omega - b + e'}</InlineMath>. Using the equation <InlineMath>{'\\frac{dq}{dt} = \\frac{1}{2}q\\omega'}</InlineMath>, we can then get our predicted quaternion for the current time using Euler's method via <InlineMath>{'q_{pred} = \\text{normalize}(q_{t - 1} + \\Delta t * \\frac{1}{2} q_{t - 1}(\\Omega - b + e))'}</InlineMath></p>
                <br/>
        ...
        <InfoDropdown outerString = 'Madgwick Filter'>
            <div className = 'flex flex-col justify-items items-center'>
                ...
                <p className = 'w-full'>With gradients calculated, we can get a change quaternion that contains the changes we believe need to be made based on the gradients, as well as our gyroscope measurements. Specifically, <InlineMath>{'dq = \\frac{1}{2} q\\Omega - \\alpha(\\nabla e_a + \\nabla e_m)'}</InlineMath>, whereby <InlineMath>{'\\Omega'}</InlineMath> are our gyroscope measurements and <InlineMath>{'q_t = \\text{normalize}(q_{t - 1} + dq * \\Delta t)'}</InlineMath></p>
                <br/>
